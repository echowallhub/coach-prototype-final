...
export class Tutorial extends React.Component{
  ...
  viewInc(id){
    fetch(`http://dev.syudo.site/video/${id}/viewed`, {
      method: 'PUT',
  ...
  showVideos(){
    return this.state.videos.map( (video, index) =>{
        return(
          <Link to={'/video'+video.url} style={{marginTop: '20px'}} className="row" key={index} onClick={() => this.viewInc(video._id)}>
            ...
            <div className="col-xs-7">
              <div style={{marginTop: '50px', marginLeft: '30px'}}>
                ...
                <p>{this.showCount(video.view_count)}</p>
              </div>
            </div>
          </Link>
        )
